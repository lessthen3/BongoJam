...
namespace BongoJam {
		...
		BongoInterpreter
	{
			...
			AddNewScope()
		{
			...
			m_ListOfCurrentlyScopedLeashes.push_back(vector<unique_ptr<int>>());
			m_ListOfCurrentlyScopedBorrowers.push_back(vector<unique_ptr<int>>());
			...
			RunBongoScript(const string& fp_BongoScriptName)
		{
			...
			for (size_t _p = 0; _p < f_Size; _p++)
			{
				...
				case 0x02: //function call
				{
					...
					//idk if this is cocher LMAO - it wasn't so i changed it lmao, apparently _n < _n + C is always true LOL, where C > 0. who would've ever thought
				...
				default:
					//THROW ERROR
					LogAndPrint("INTERNAL COMPILER ERROR: Error at BYTE-CODE: " + to_string(f_ByteCode[_p]), "Interpreter", "error", "red");
