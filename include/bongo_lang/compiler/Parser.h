...
namespace BongoJam {
    ...
    public:
        Parser()
        {
            ...
            parser_logger->Initialize("./logs", "ParsingLogger", Logger::LogLevel::ALL_LOGS);

            parser_logger->Info("Parser Logger intialized properly!", "Parser");
    ...
    private:
            ...
            )
        {
            ...
            case TokenType::Dot: //used for class calling, eg. "myClassInstance.myMethod(func args);" or "myClassInstance.myField = 69;"
            {
                ...
                {
                    //THROW ERROR
                    parser_logger->Error("Error at Line Number: " + to_string(fp_CurrentToken.m_SourceCodeLineNumber), "Parser");
        ...
        bool
            ...
            (
                ...
                vector<Token>& fp_ProgramTokens, 
                size_t& fp_ProgramCounter, 
                IfDeclaration* fp_IfStatement, 
                IfDeclaration* fp_ElseIfStatement = nullptr
            )
        {
            ...
            IfDeclaration* f_CurrentStatement = nullptr; //either an if or else-if statement
            ...
            while (fp_ProgramTokens.size() > 0 and f_BracketDepthTracker > 0)
            {
                ...
                case TokenType::Break:
                {
                    ...
                    //f_CurrentStatement->m_CodeBody.emplace_back(BreakSubStatement());
                }
                break;
                case TokenType::If:
                {
                    unique_ptr<IfDeclaration> s_NestedIfStatement;

                    if (not ParseIfBlock(fp_CurrentScopeDepth, fp_IsInLoop, fp_CurrentToken, fp_ProgramTokens, fp_ProgramCounter, s_NestedIfStatement.get()))
                    {
            ...
            {
                IfDeclaration f_FallThroughCondition;
            ...
            {
                ElseDeclaration f_ElseStatement;
            ...
            )
        {
            ...
            if (fp_CurrentToken.m_Type != TokenType::OpenBracket)
            {
                //THROW ERROR
                parser_logger->Error("Error at Line Number: " + to_string(fp_CurrentToken.m_SourceCodeLineNumber), "Parser");
                parser_logger->Warning("Unrecognized symbol found in your else-block brother! Try taking a look at your '{ }' code-body wrappers", "Parser");
                return false;
            }
            ...
            )
        {
            ...
            while (f_StillInFunctionArgDefinition)
            {
                ...
                if (fp_CurrentToken.m_Type != TokenType::UserIdentifier) //handles variables, function, and class instance names being passed as a single argument, could use this for some semi-dynamic typing xdxd
                {
                    //THROW ERROR
                    parser_logger->Error("Error at Line Number: " + to_string(fp_CurrentToken.m_SourceCodeLineNumber), "Parser");
                    parser_logger->Warning("Unrecognized symbol following an open parenthesis while declaring: " + fp_FuncBlock->m_FuncName.m_Value + "'s arguments brother! Try taking a look at your function argument(s) defintion", "Parser");
                    return false;
                }

                fp_FuncBlock->m_FuncArgs.push_back(move(make_unique<SingleValueExpr>(fp_CurrentToken)));
                fp_CurrentToken = ShiftForward(fp_ProgramTokens); //shift forward to look for an open paren
                fp_ProgramCounter++;
                ...
                if (fp_CurrentToken.m_Type != TokenType::TypeArrow)
                {
                    //THROW ERROR
                    parser_logger->Error("Error at Line Number: " + to_string(fp_CurrentToken.m_SourceCodeLineNumber), "Parser");
                    parser_logger->Warning("Unrecognized symbol following a name definition while declaring: " + fp_FuncBlock->m_FuncName.m_Value + "'s arguments brother! Try taking a look at your type-arrows", "Parser");
                    return false;
                }
                    ...
                    )
                {
                    //THROW ERROR
                    parser_logger->Error("Error at Line Number: " + to_string(fp_CurrentToken.m_SourceCodeLineNumber), "Parser");
                    parser_logger->Warning("Unrecognized type found while declaring: " + fp_FuncBlock->m_FuncName.m_Value + "'s brother! Are you sure you've entered a valid type in " + fp_FuncBlock->m_FuncName.m_Value + "'s argument definition?", "Parser");
                    return false;
                }

                fp_FuncBlock->m_FuncArgTypes.push_back(move(make_unique<SingleValueExpr>(fp_CurrentToken))); //type indices correspond to the func arg list
                fp_CurrentToken = ShiftForward(fp_ProgramTokens); //shift forward to look for an open paren
                fp_ProgramCounter++;
                ...
                if (fp_CurrentToken.m_Type != TokenType::Comma)
                {
                    ...
                    else
                    {
                        //THROW ERROR
                        parser_logger->Error("Error at Line Number: " + to_string(fp_CurrentToken.m_SourceCodeLineNumber), "Parser");
                        parser_logger->Warning("Unexpected symbol found in: " + fp_FuncBlock->m_FuncName.m_Value + "'s argument defintion! \n Try taking a look at your comma separation between function parameters", "Parser");
                        return false;
                    }
                } //end of scope
                else if (fp_CurrentToken.m_Type == TokenType::ENDF)
                {
                    //THROW ERROR
                    parser_logger->Error("Error at Line Number: " + to_string(fp_CurrentToken.m_SourceCodeLineNumber), "Parser");
                    parser_logger->Warning("End of file found instead of a closed parenthesis inside: " + fp_FuncBlock->m_FuncName.m_Value + "'s definition! \n Make sure you end your function parameter definition with a ')'", "Parser");
                    return false;
                }
            }
            ...
            if (fp_CurrentToken.m_Type != TokenType::TypeArrow)
            {
                //THROW ERROR
                parser_logger->Error("Error at Line Number: " + to_string(fp_CurrentToken.m_SourceCodeLineNumber), "Parser");
                parser_logger->Warning("Unexpected symbol found in: " + fp_FuncBlock->m_FuncName.m_Value + "'s return type defintion! \n Try taking a look at your type-arrow definition", "Parser");
                return false;
            }
            else
            {
               ...
               )
            {
                //THROW ERROR
                parser_logger->Error("Error at Line Number: " + to_string(fp_CurrentToken.m_SourceCodeLineNumber), "Parser");
                parser_logger->Warning("Invalid type found in: " + fp_FuncBlock->m_FuncName.m_Value + "'s return type defintion! \n Try taking a look at your type-arrow definition", "Parser");
                return false;
            }
            else
            {
            ...
            if (fp_CurrentToken.m_Type != TokenType::OpenBracket)
            {
                //THROW ERROR
                parser_logger->Error("Error at Line Number: " + to_string(fp_CurrentToken.m_SourceCodeLineNumber), "Parser");
                parser_logger->Warning("Unrecognized symbol following " + fp_FuncBlock->m_FuncName.m_Value + "'s function parameter definition! Try taking a look at your function body definition", "Parser");
                return false;
            }
            else
            {
            ...
            while (fp_CurrentToken.m_Type != TokenType::CloseBracket and fp_ProgramTokens.size() > 0)
            {
                ...
                case TokenType::Print:
                {
                    ...
                    if (not ParsePrintFunction(fp_CurrentScopeDepth, fp_CurrentToken, fp_ProgramTokens, fp_ProgramCounter, s_PrintFunctionCall.get()))
                    {
                        parser_logger->Error(format("Parsing Error at line {}, invalid call to print() function", fp_CurrentToken.m_SourceCodeLineNumber), "Parser");
                ...
                case TokenType::Var:
                {
                    VarDeclaration s_VariableDefinition;

                    if (not ParseVarDeclaration(fp_CurrentScopeDepth, fp_CurrentToken, fp_ProgramTokens, fp_ProgramCounter, fp_Program, &s_VariableDefinition))
                    {
                        fp_ProgramTokens.clear(); //dump source code so that parsing ends immediately
                        return false;
                    }
                ...
                case TokenType::If:
                {
                    ...
                    if (not ParseIfBlock(fp_CurrentScopeDepth, false, fp_CurrentToken, fp_ProgramTokens, fp_ProgramCounter, s_IfStatement.get()))
                    {
                        fp_ProgramTokens.clear(); //dump source code so that parsing ends immediately
                        continue;
                    }
        ...
        bool //returns true if it worked, false if it failed
            ...
            (
                ...
                size_t& fp_ProgramCounter, 
                Program* fp_Program, 
                VarDeclaration* fp_VariableDefinition
            )
        {
            fp_CurrentToken = ShiftForward(fp_ProgramTokens); //shift forward one token to check for any accessor symbols
            fp_ProgramCounter++;
            ...
            while (fp_ProgramTokens.size() > 0 and fp_CurrentToken.m_Type != TokenType::SemiDot) //parse equality expression
            {
                ...
                case TokenType::UserIdentifier: //used for: var x -> Car: new Car();, var x -> Car = new Car();
                {
                    ...
                    //    //THROW ERROR
                    //    parser_logger->Error("Error at Line Number: " + to_string(fp_CurrentToken.m_SourceCodeLineNumber), "Parser");
                    //    parser_logger->Warning("Undefined class type found while declaring a variable brother! Are you defining this variable before your class definition?", "Parser");
                    //    return false;
                    //}
                }
                break;
        ...
        bool
            ...
            (
                ...
                vector<Token>& fp_ProgramTokens, 
                size_t& fp_ProgramCounter, 
                PrintFunction* fp_PrintFunctionCall
            )
        {
            ...
            {
                fp_PrintFunctionCall->m_FuncArgs[0] = (make_unique<SingleValueExpr>(fp_CurrentToken)); //index 0 for the first arg of print
            ...
            if (fp_CurrentToken.m_Type == TokenType::Comma)
            {
                ...
                {
                    fp_PrintFunctionCall->m_FuncArgs[1] = make_unique<SingleValueExpr>(fp_CurrentToken); //colour arg, so the 1st index
                    fp_CurrentToken = ShiftForward(fp_ProgramTokens); //shift forward one token to check for a type arrow
                    fp_ProgramCounter++;
                }
            }
            ...
            ConstructAST(vector<Token>& fp_ProgramTokens)
        {
            ...
            while (not IsENDF(f_CurrentToken) and fp_ProgramTokens.size() > 0) //this loop operates on every statement that lives on scope-depth = 0, everything else is branched like a tree from the 0th level
            {
                ...
                case TokenType::Func: //used for func and class method definitions
                {
                    ...
                    if (not ParseFuncDeclaration(f_CurrentScopeDepth, f_CurrentToken, fp_ProgramTokens, f_Program.get(), f_ProgramCounter, s_FunctionDefintion.get()))
                    {
                        parser_logger->Error(format("Parsing Error at line {}, function declaration is invalid", f_CurrentToken.m_SourceCodeLineNumber), "Parser");
                        fp_ProgramTokens.clear(); //dump source code so that parsing ends immediately
                        continue;
                    }
                    else
                    {
                        f_Program->ParsedScript.push_back(move(s_FunctionDefintion));
                ...
                default:
                {
                    parser_logger->Error("Compiler Error: Improper grammar found in source code at line " + to_string(f_CurrentToken.m_SourceCodeLineNumber), "Parser");
