...
namespace BongoJam {
	...
	private:
			...
			) 
		{
			...
			if (not f_BongoCode) 
			{
				runtime_logger->LogAndPrint("Fatal Error: Interpreter failed to open .bongo file for reading!", "Interpreter", Logger::LogLevel::Fatal);
				return false;
			}
	...
	public:
			...
			RunBongoScript(const string& fp_BongoScriptName)
		{
			...
			for (size_t _p = 0; _p < f_Size; _p++)
			{
				...
				default:
					//THROW ERROR
					runtime_logger->LogAndPrint("INTERNAL COMPILER ERROR: Error at Line Number: "+to_string(_l)+", Error at BYTE-CODE: "+to_string(f_ByteCode[_p]), "Interpreter", Logger::LogLevel::Error);
					runtime_logger->LogAndPrint("Something terrible happened while running the code, invalid bytecode was generated by the compiler (sorry not your fault I think LOL)", "Interpreter", Logger::LogLevel::Warning);
